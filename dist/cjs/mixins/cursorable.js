...
function cursorable(Base, config) {
    ...
    function getEqualOperator(cursorPart, isReversed) {
        ...
        if (isReversed && reversable) {
            return direction === 'ASC' ? (0, kysely_1.sql) `<` : (0, kysely_1.sql) `>`;
        }
        return direction === 'ASC' ? (0, kysely_1.sql) `>` : (0, kysely_1.sql) `<`;
    }
            ...
            return {
                ...
                totalCount: async () => {
                    ...
                    const { count } = await this
                        .selectFrom()
                        .if(!!func, (qb) => func?.(qb))
                        .select((0, kysely_1.sql) `count(${kysely_1.sql.literal(`${this.table}.${this.id}`)})`.as('count'))
